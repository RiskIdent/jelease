...
// NewRelease object unmarshaled from the newreleases.io webhook.
...
func (newRelease NewRelease) JiraIssue() jira.Issue {
...
func handlePostWebhook(w http.ResponseWriter, r *http.Request) {
	...
	previousIssuesQuery := fmt.Sprintf("status = %q and labels = %q", config.DefaultStatus, newRelease.Project)
	...
	for _, previousIssue := range previousIssues {
		...
		fmt.Printf("Updated issue summary from %q to %q", previousSummary, newRelease.IssueSummary())
	}
}
